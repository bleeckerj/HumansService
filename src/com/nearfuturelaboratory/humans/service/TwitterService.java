...
public class TwitterService {
	...
	// private static final String STATUS_DB_PATH =
	// Constants.getString("SERVICE_DATA_ROOT")+"/twitter/users/%s-%s/status/";
	// private static final String USERS_DB_PATH_ROOT =
	// Constants.getString("SERVICE_DATA_ROOT")+"/twitter/users/";
	// private static final String USERS_DB_PATH =
	// Constants.getString("SERVICE_DATA_ROOT")+"/twitter/users/%s-%s/";
	// private static final String USERS_FOLLOWS_PATH =
	// Constants.getString("SERVICE_DATA_ROOT")+"/twitter/users/%s-%s/follows/";
	//
	// private static final String USERS_DB_PATH_CODED =
	// Constants.getString("SERVICE_DATA_ROOT")+"/twitter/users/%s/";
	// private static final String USERS_SER_TOKEN =
	// USERS_DB_PATH+"twitter-token-for-%s-%s.ser";

	// private static final long STATUS_STALE_TIME =
	// Constants.getLong("STATUS_STALE_TIME");
	// private static final long USER_BASIC_STALE_TIME =
	// Constants.getLong("USER_BASIC_STALE_TIME");

	protected Gson gson;

	// public static TwitterService
	// createTwitterServiceOnBehalfOfCodedUsername(String aCodedUsername) {
	 ...
	 * logger.debug(result+" "+"self"); this.user = result; if(save) { String
	 * username = (String)result.get("screen_name"); // String path =
	 * String.format(USERS_DB_PATH, aUserID, username); File f = new
	 * File(String.format(USERS_DB_PATH, aUserID, username)); String p =
	 * (String)user.get("id_str")+"-"+username+".json";
	 * 
	 * 
	 * writeJSONToFile(result, f, p); } return result; }
	 */
	...
	// com.nearfuturelaboratory.humans.entities.TwitterUser tuser =
	// gson.fromJson(aUserJson.toString(),
	...
	public List<TwitterStatus> serviceRequestStatusForUserID(String aUserID) {
		...
		TwitterStatus most_recent = statusDAO
				.findMostRecentStatusByExactUserID(aUserID);
		if (most_recent == null) {
			result = this
					.serviceRequestStatusForUserIDAndSinceID(aUserID, null);
		} else {
	...
	protected List<TwitterStatus> serviceRequestStatusForUserIDAndSinceID(
		...
		logger.debug("Twitter: Getting " + serviceUserTwitterUsername
				+ " on behalf of " + this.getThisUser().getScreen_name());
		// logger.info("From Headers for Twitter Request rate-limit="+h.get("x-rate-limit-limit")+" reset ms="+h.get("x-rate-limit-reset"));

		// JSONObject new_obj = (JSONObject)jsonResponse.get(0);
		// JSONObject old_obj =
		// (JSONObject)jsonResponse.get(jsonResponse.size()-1);
		// // long newest_id = JsonPath.read(new_obj, "id");
		// long oldest_id = JsonPath.read(old_obj, "id");
		// writeJSONToFile(jsonResponse, new File(path),
		// aUserID+"-"+oldest_id+"-"+newest_id+"-status.json");
	...
	public List<TwitterStatus> getStatusForUserID(String aUserID) {
		...
		// Gson gson = new
		// GsonBuilder().setDateFormat("EEE MMM dd HH:mm:ss ZZZZZ yyyy").create();
		// try {
		//
		// InputStreamReader char_input =
		// new InputStreamReader(new
		// FileInputStream(f),Charset.forName("UTF-8").newDecoder());
		//
		...
		// TwitterStatus ts = gson.fromJson(statuses.get(j),
		// TwitterStatus.class);
	...
	List<TwitterStatus> saveStatusJson(JSONArray status) {
		...
		while (iter.hasNext()) {
			...
			// logger.debug(i);
			com.nearfuturelaboratory.humans.twitter.entities.TwitterStatus tstatus = gson
					.fromJson(
							i,
							com.nearfuturelaboratory.humans.twitter.entities.TwitterStatus.class);
			result.add(tstatus);
	...
	// JSONArray result;
	// JSONParser parser = new JSONParser();
	// File f = new File(String.format(USERS_DB_PATH_CODED,
	// aCodedUsername)+"/follows/"+aCodedUsername+"-hydratedfollows.json");
	...
	public List<TwitterFollows> getFriends() {
		return getFriendsFor(this.getThisUser().getId());
	}

	protected List<TwitterFollows> getFriendsFor(String aUserId) {
		List<TwitterFollows> follows = new ArrayList<TwitterFollows>();
		follows = followsDAO.findFollowsByExactUserID(this.getThisUser().getId());
		return follows;
	}


	public boolean localFollowsIsFresh() {
	...
	protected boolean localFollowsIsFreshFor(String aUserID) {
		...
		TwitterFollows follows = followsDAO.findOldestFriendByExactUserID(aUserID);
	...
	protected List<TwitterFollows> saveFollows(List<JSONObject> list_of_friends, String follower_id) {

		List<TwitterFollows> new_friends= new ArrayList<TwitterFollows>();
		for(JSONObject j : list_of_friends) {
			...
			TwitterFollows f = followsDAO.findFollowsByUserIDFollowsID(friend.getId(), follower_id);
			if(f == null) {
				f = new TwitterFollows();
				f.setFriend(friend);
		...
		List<TwitterFollows> existing_friends = this.getFriendsFor(follower_id);

		Collection<TwitterFollows> new_friends_to_save = CollectionUtils.subtract(new_friends, existing_friends);
		Collection<TwitterFollows> no_longer_friends = CollectionUtils.subtract(existing_friends, new_friends);

		// delete the ones that have dropped off
		for(TwitterFollows not_a_friend : no_longer_friends) {
			followsDAO.delete(not_a_friend);
			// but keep the user around?
			//
		}
		
		// okay, because TwitterFollows is a weird data structure, we need to
		...
		for(TwitterFollows is_a_friend : new_friends_to_save) {
